...
public static class XLocalization 
{
	...
	static string defaultLanguage = "English (US)";
	...
	public static string language
	{
		...
		set
		{
			...
			foreach(GameObject obj in localizeGameObjects)
			{
                if (obj != null)
                {
                    if (obj.GetComponent<XLocalize>())
                    {
                        obj.GetComponent<XLocalize>().LoadKey();
                    }
                    else
                    {
                        obj.GetComponent<XLocalizeTextMesh>().LoadKey();
                    }
                }
			}
		}
	...
	public static void LoadLocalization()
	{
		if (!localizationLoaded)
		{
			...
			// If Localization file doesn't exist, we load english default
			if (textAsset == null)
			{
				Debug.LogWarning("[Localization] Localization not found: "+filePath+currentLanguage);
				textAsset = (TextAsset)Resources.Load(filePath + defaultLanguage, typeof(TextAsset));
				//return;
			}
